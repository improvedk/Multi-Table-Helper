...
namespace MTH.Framework
{
	public abstract class Win32
	{
		...
		public extern static bool PrintWindow (int hWnd, int dc, uint reservedFlag);

		public static Bitmap PrintWindowAsBitmap(int hWnd, int width, int height)
		{
			int hDC = Win32.GetWindowDC(hWnd);
			int hDCDest = Win32.CreateCompatibleDC(hDC);
			int hBitmap = Win32.CreateCompatibleBitmap(hDC, width, height);
			int hOld = Win32.SelectObject(hDCDest, hBitmap);
			Win32.PrintWindow(hWnd, hDCDest, 0);
			Win32.SelectObject(hDCDest, hOld);
			Win32.DeleteDC(hDCDest);
			Win32.ReleaseDC(hWnd, hDC);
			Bitmap screenshot = Bitmap.FromHbitmap((IntPtr)hBitmap);
		...
		public delegate bool PCallBack(int hwnd, int lParam);

		public static string GetModuleDirectory(int hWnd)
		{
			return Path.GetDirectoryName(Process.GetProcessById(Win32.GetWindowProcessID(hWnd)).MainModule.FileName);
		}
		...
		public static extern int SelectObject(int hdc, int bmp);
        ...
        [DllImport("user32.dll")]
        private static extern void mouse_event(UInt32 dwFlags, UInt32 dx, UInt32 dy, UInt32 dwData, int dwExtraInfo);
		...
		[DllImport("kernel32")]
		public static extern int VirtualAllocEx(int hProcess, int lpAddress, int dwSize, uint flAllocationType, uint flProtect);

		[DllImport("kernel32")]
		public static extern int VirtualAllocEx(long hProcess, int lpAddress, int dwSize, uint flAllocationType, uint flProtect);
		...
		public static extern bool ReadProcessMemory( int hProcess, int lpBaseAddress, 
			int lpBuffer, int dwSize, int lpNumberOfBytesRead );
		...
		[DllImport("user32")]
		public static extern int GetWindowThreadProcessId(Int32 hWnd, out Int32 lpdwProcessId);

		[DllImport("user32")]
		public static extern int GetWindowThreadProcessId(long hWnd, out long lpdwProcessId);
		...
		[DllImport("user32.dll")]
		public static extern int SendMessage(int hWnd, Int32 msg, Int32 wParam, int lParam);
		...
		[DllImport("user32.dll")]
		public static extern int SendMessage(int hWnd, Int32 msg, Int32 wParam, ref Rectangle rect);

		[DllImport("user32.dll")]
		public static extern int SendMessage(int hWnd, Int32 msg, bool wParam, ref Rectangle rect);
		...
		[DllImport("user32.dll")]
		public static extern int ReleaseDC(int windowHwnd, int hdcHwnd);
		...
		public static extern bool BitBlt(
			int hdcDest, // handle to destination DC
			int nXDest, // x-coord of destination upper-left corner
			int nYDest, // y-coord of destination upper-left corner
			int nWidth, // width of destination rectangle
			int nHeight, // height of destination rectangle
			int hdcSrc, // handle to source DC
		...
		public static extern int CreateDC(
			 ...
			 int lpInitData	// optional printer data
				  );
		...
		[DllImport("user32.dll")]
		public static extern bool RedrawWindow(int hWnd, ref Rectangle lprcUpdate, int hrgnUpdate, uint flags);
